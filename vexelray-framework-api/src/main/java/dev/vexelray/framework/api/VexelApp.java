 ...
 *
 * <p>One per application. The processor generates a wiring class beside it, and the annotated class's
 * {@code main} becomes a single call into the runtime:
 *
 * <pre>{@code
...
public @interface VexelApp {
     ...
     * renamed or is not on the path is a <b>compile error</b> rather than a component missing at startup. The
     * processor obtains the type by resolution rather than search, so build time is proportional to what is
     * used. And the set of things configuring this application is written at the one place somebody reads when
     * they want to know.
     *
