...
| **mainframe** | A terminal/shell. Its `mainframe-template` is now only the adapter (`…template.shell`) that puts **this repo's** project builder behind `new vexel-desktop` — the engine and the template moved here as `vexelray-framework-template`, and mainframe depends on it, so a change to the builder's public API is a change mainframe compiles against. **Read the template's `files/App.java` and `files/Wiring.java`** — they are what a correct application looks like. mainframe is also a library importing `WindowMemory` and `Settings`, so it is inside the absorption blast radius |
